...
# Quick development script for Matcha Docker TF2
# Usage: ./dev.sh [build|run|stop|clean|logs|shell]

set -e

IMAGE_NAME="matcha-docker-tf2"
CONTAINER_NAME="dev"
TAG="dev"
...
case "$1" in
    ...
    run)
        ...
        docker run -dit \
            ...
            --net=host \
            -e GH_PAT="${GH_PAT:-}" \
            -e MATCHA_API_KEY="${MATCHA_API_KEY:-}" \
            ${IMAGE_NAME}:${TAG}
    ...
    *)
        ...
        echo "  MATCHA_API_KEY - Matcha API Key"
        echo ""
        echo "Examples:"
        echo "  export GH_PAT='your_token_here'"
        echo "  export MATCHA_API_KEY='your_api_key_here'"
        echo "  $0 rebuild"
        exit 1
        ;;
esac
